...
# Product Class
# =============================================================================
# Define product constants for clarity. Here, only SQUID_INK is in use.
class Product:
    SQUID_INK = "SQUID_INK"
...
# PARAMS Dictionary
# =============================================================================
# This dictionary holds key trading parameters for SQUID_INK. Each parameter is
...
#
# base_fair_value:
#   - The starting estimate of the true market value of SQUID_INK based on historical data.
...
PARAMS = {
    Product.SQUID_INK: {
        ...
        "aggressiveness": 0.5,           # 0.0 to 1.0, higher = more aggressive
        "fair_value_order_size": 0,      # Size of orders placed at fair value
        "tier1_distance": 2,             # Distance of first tier from fair value
        "tier2_distance": 4,             # Distance of second tier from fair value  
        "tier3_distance": 6,             # Distance of third tier from fair value
        "tier1_volume_pct": 0.6,         # Percentage of available volume for tier 1
        "tier2_volume_pct": 0.3,         # Percentage of available volume for tier 2
        "tier3_volume_pct": 0.1,         # Percentage of available volume for tier 3
        "position_scaling": 0.2,         # How much to adjust volume distribution based on position
    },
}
...
class Trader:
    def __init__(self, params=None):
        ...
        # Limit for the maximum allowed position for SQUID_INK. This is the hard limit,
    ...
    # -------------------------------------------------------------------------
    def calculate_fair_value_SQUID_INK(
        self,
        product: str,
        ...
        Parameters:
        - product: The trading instrument (here, SQUID_INK).
    ...
    # -------------------------------------------------------------------------
    def take_best_orders_SQUID_INK(
        self,
        product: str,
        fair_value: int,
        take_width: float,
        ...
        Parameters:
        - product: Trading instrument (SQUID_INK).
        ...
        - position: Current net position in SQUID_INK.
        - buy_order_volume: Running total of SQUID_INK already being bought this round.
        - sell_order_volume: Running total of SQUID_INK already being sold this round.
    ...
    def market_make_SQUID_INK(
        ...
        - position: Current net position of SQUID_INK.
    ...
    # -------------------------------------------------------------------------
    def clear_position_order_SQUID_INK(
        self,
        product: str,
        fair_value: float,
        width: int,
    ...
    def take_orders_SQUID_INK(
        ...
        buy_order_volume, sell_order_volume = self.take_best_orders_SQUID_INK(
            product,
            fair_value,
            take_width,
            orders,
    ...
    def clear_orders_SQUID_INK(
        ...
        buy_order_volume, sell_order_volume = self.clear_position_order_SQUID_INK(
            product,
            fair_value,
            clear_width,
            orders,
    ...
    # -------------------------------------------------------------------------
    # Market Making with Dynamic Pricing for SQUID_INK
    # -------------------------------------------------------------------------

    def make_SQUID_INK_orders_SQUID_INK(
        self,
        order_depth: OrderDepth,
        fair_value: float,
        position: int,
        ...
        aggressiveness = self.params[Product.SQUID_INK].get("aggressiveness", 0.5)
        fair_value_order_size = self.params[Product.SQUID_INK].get("fair_value_order_size", 5)
        tier1_distance = self.params[Product.SQUID_INK].get("tier1_distance", 1)
        tier2_distance = self.params[Product.SQUID_INK].get("tier2_distance", 2)
        tier3_distance = self.params[Product.SQUID_INK].get("tier3_distance", 3)
        tier1_volume_pct = self.params[Product.SQUID_INK].get("tier1_volume_pct", 0.5)
        tier2_volume_pct = self.params[Product.SQUID_INK].get("tier2_volume_pct", 0.3)
        tier3_volume_pct = self.params[Product.SQUID_INK].get("tier3_volume_pct", 0.2)
        position_scaling = self.params[Product.SQUID_INK].get("position_scaling", 0.2)
        
        # Calculate position ratio (-1 to 1 scale)
        position_ratio = position / self.LIMIT[Product.SQUID_INK]
        ...
        available_buy_quantity = self.LIMIT[Product.SQUID_INK] - (position + buy_order_volume)
        available_sell_quantity = self.LIMIT[Product.SQUID_INK] + (position - sell_order_volume)
        ...
        if aggressiveness > 0.7 and fair_value_buy_size > 0 and remaining_buy_qty > 0:
            ...
            orders.append(Order(Product.SQUID_INK, round(fair_value), actual_qty))
        ...
        if remaining_buy_qty > 0:
            ...
            if tier1_qty > 0:
                orders.append(Order(Product.SQUID_INK, round(fair_value - effective_tier1_distance), tier1_qty))
        ...
        if remaining_buy_qty > 0:
            ...
            if tier2_qty > 0:
                orders.append(Order(Product.SQUID_INK, round(fair_value - effective_tier2_distance), tier2_qty))
                remaining_buy_qty -= tier2_qty
                buy_order_volume += tier2_qty
        
        # Tier 3: Furthest from fair value
        if remaining_buy_qty > 0:
            orders.append(Order(Product.SQUID_INK, round(fair_value - effective_tier3_distance), remaining_buy_qty))
        ...
        # Tier 3: Furthest from fair value
        if remaining_sell_qty > 0:
            orders.append(Order(Product.SQUID_INK, round(fair_value + effective_tier3_distance), -remaining_sell_qty))
    ...
    def run(self, state: TradingState):
        ...
        Main entry point of the trading strategy. This method:
         ...
         2. Checks if SQUID_INK is available for trading.
        ...
        # Verify that SQUID_INK is tradable in the current state.
        if Product.SQUID_INK in self.params and Product.SQUID_INK in state.order_depths:
            # Retrieve the current net position for SQUID_INK.
            SQUID_INK_position = (
            ...
            fair_value = self.calculate_fair_value_SQUID_INK(
                Product.SQUID_INK,
            ...
            current_spread = traderObject.get(f"{Product.SQUID_INK}_current_spread", 3)
            ...
            recent_prices = [data["mid_price"] for data in traderObject.get(f"{Product.SQUID_INK}_price_data", [])]
            if len(recent_prices) >= 10:
                ...
                dynamic_soft_limit = int(self.params[Product.SQUID_INK]["soft_position_limit"] * volatility_factor)
                ...
                dynamic_soft_limit = self.params[Product.SQUID_INK]["soft_position_limit"]
            
            # Log key metrics for debugging purposes.
            #print(f"SQUID_INK - Fair Value: {fair_value}, Position: {SQUID_INK_position}, Spread: {current_spread}")
            ...
            SQUID_INK_take_orders, buy_order_volume, sell_order_volume = self.take_orders_SQUID_INK(
                Product.SQUID_INK,
                state.order_depths[Product.SQUID_INK],
                fair_value,
            ...
            SQUID_INK_clear_orders, buy_order_volume, sell_order_volume = self.clear_orders_SQUID_INK(
                Product.SQUID_INK,
                state.order_depths[Product.SQUID_INK],
                fair_value,
            ...
            SQUID_INK_make_orders, _, _ = self.make_SQUID_INK_orders_SQUID_INK(
                state.order_depths[Product.SQUID_INK],
                fair_value,
                SQUID_INK_position,
                buy_order_volume,
                ...
                self.params[Product.SQUID_INK]["penny_val"],
            )
            '''
            # Combine all generated orders for SQUID_INK into the result.
            result[Product.SQUID_INK] = (
                SQUID_INK_take_orders + SQUID_INK_clear_orders ##+ SQUID_INK_take_orders_SQUID_INK + 
            )
            #print(f"SQUID_INK Orders: {result[Product.SQUID_INK]}")
