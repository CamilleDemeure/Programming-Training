...
PARAMS = {
    Product.KELP: {
        "base_fair_value": 2033,      # Baseline fair value near observed median prices (~2033)
        "take_width": 4,              # Distance from fair value to capture mispriced orders; increased for wider capture
        "clear_width": 1,             # Distance threshold for triggering position clearing due to volatility
        "disregard_edge": 1,          # Orders too close (within 1 unit) to fair value are ignored for pennying/joining
        "join_edge": 2,               # Use joining tactics when orders fall within 2 units from fair value
        "default_edge": 3,            # When no viable orders are in range, default to a 3 unit offset from fair value
        "soft_position_limit": 45,    # Conservative position management target, lower than hard limit (50)
        "volume_limit": 0,            # Volume management parameter placeholder (can be adjusted if needed)
        "penny_val": 1,               # Price adjustment amount when trying to outbid existing orders (pennying)
        "vwap_window": 20,            # Number of data points to average for a more responsive VWAP-derived fair value
    },
}
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            result[Product.KELP] = (
                kelp_make_orders +kelp_take_orders + kelp_clear_orders
            )
