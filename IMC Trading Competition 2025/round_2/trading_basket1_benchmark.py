...
'''
PICNIC_BASKET1 Statistical Arbitrage Trading Strategy Explanation

Overview
This trading strategy implements a statistical arbitrage approach for PICNIC_BASKET1 and its components.
The strategy exploits price differences between the actual basket product and its synthetic equivalent
created from the individual components (CROISSANTS, JAMS, and DJEMBES). By detecting when these prices
...
The strategy begins by creating a synthetic version of PICNIC_BASKET1:

Basket Composition: One PICNIC_BASKET1 consists of 4 CROISSANTS, 6 JAMS, and 1 DJEMBES.
...
PARAMS = {
    Product.SPREAD: {
        # Update the mean to match the observed data
        "default_spread_mean": 37.441800,   # From the dataset mean in missprice_1
        # Standard deviation is close to observed data (85.15)
        "default_spread_std": 85.148401,      # From standard deviation in missprice_1
        # Keep this parameter or adjust based on available memory/performance needs
        "spread_std_window": 50,          # Number of observations to include in rolling std calculation
        # Adjust based on desired trade frequency/confidence level
        "zscore_threshold": 10,          # Slightly more conservative than before        
        # This could remain the same, but consider adjusting based on risk appetite
        "target_position": 58,            # Slightly below the position limit of 60 for safety
    },
}

# The weights of each component in one basket
# These define the composition of PICNIC_BASKET1
BASKET_WEIGHTS = {
    Product.CROISSANTS: 6,      # Each basket contains 6 croissants
    Product.JAMS: 3,            # Each basket contains 3 jams
    Product.DJEMBES: 1,         # Each basket contains 1 djembe
}
...
class Trader:
    def __init__(self, params=None):
        ...
        self.LIMIT = {
            Product.PICNIC_BASKET1: 60,    # Maximum 60 baskets long or short
            Product.CROISSANTS: 250,       # Maximum 250 croissants long or short
            Product.JAMS: 350,             # Maximum 350 jams long or short
            Product.DJEMBES: 60,           # Maximum 60 djembes long or short
        }
    ...
    def spread_orders(
        ...
        if (
            ...
            return self.params[Product.SPREAD]["default_spread_std"]
