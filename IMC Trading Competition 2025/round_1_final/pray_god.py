...
#############################################
# Optimized TraderResin (for RAINFOREST_RESIN trading)
#############################################
...
class TraderResin:
    ...
    def market_make(self,
        ...
        penny_val = self.params.get("penny_val", 1)
        default_edge = self.params.get("default_edge", 4)
        volume_limit = self.params.get("volume_limit", 0)
    ...
    def run(self, state: TradingState, traderObject: Dict) -> Tuple[Dict, int, Dict]:
        result = {}
        if Product.RAINFOREST_RESIN in state.order_depths:
            ...
            take_orders, buy_vol, sell_vol = self.take_best_orders(
                od,
                self.params.get("fair_value", 10000),
                self.params.get("take_width", 2),
                resin_position,
                0,
                0
            )
            clear_orders, buy_vol, sell_vol = self.clear_position_order(
                od,
                self.params.get("fair_value", 10000),
                self.params.get("clear_width", 0.5),
                resin_position,
                buy_vol,
                sell_vol
            )
        ...
        return result, 0, traderObject

#############################################
# Optimized Trader_SQUID_INK (for SQUID_INK trading)
#############################################
...
class Trader_SQUID_INK:
    ...
    def predict_next_prices(self, bid_vwap: float, ask_vwap: float) -> Tuple[float, float]:
        bid_off = self.params.get("bid_offset", 0)
        ask_off = self.params.get("ask_offset", 0)
    ...
    def update_price_history(self, traderObject: Dict, product: str, state: TradingState) -> Tuple[float, float, float, float, float]:
        # We now store only the last 25 steps.
        window_size = 10  
        history_keys = [
    ...
    def take_opportunity_orders(self,
        ...
        max_position = int(limit * self.params.get("max_position_pct", 0.8))
        buy_vol = 0
        sell_vol = 0
        safety = self.params.get("safety_margin", 0.03)
        take_threshold = self.params.get("take_threshold", 0.1)
    ...
    def clear_position_orders(self,
        ...
        if abs(pos_after) < 1:
            return orders, buy_vol, sell_vol
        clear_threshold = self.params.get("clear_threshold", 0.02)
    ...
    def market_making_orders(self,
        ...
        pos_scaling = self.params.get("position_scaling", 0.8)
        position_scale = 1 - pos_ratio * pos_scaling
        position_scale = max(0.1, position_scale)
        make_threshold = self.params.get("make_threshold", 0.05)
        if pos_after > 0:
    ...
    def run(self, state: TradingState, traderObject: Dict = None) -> Tuple[Dict, int, Dict]:
        ...
        if Product.SQUID_INK in state.order_depths:
            ...
            if fair_value is None and od.buy_orders and od.sell_orders:
                best_bid = max(od.buy_orders.keys())
                best_ask = min(od.sell_orders.keys())
                bid_vwap = best_bid
                ask_vwap = best_ask
                fair_value = (best_bid + best_ask) / 2
                next_bid = bid_vwap + self.params.get("bid_offset", 0)
                next_ask = ask_vwap + self.params.get("ask_offset", 0)
            squid_orders = []
            buy_vol = 0
            sell_vol = 0
            if fair_value is not None and next_bid is not None and next_ask is not None:
                opp_orders, bvol, svol = self.take_opportunity_orders(Product.SQUID_INK, od, fair_value, next_bid, next_ask, position)
                squid_orders.extend(opp_orders)
                buy_vol += bvol
                sell_vol += svol
                clear_orders, bvol, svol = self.clear_position_orders(Product.SQUID_INK, od, fair_value, position, buy_vol, sell_vol)
                squid_orders.extend(clear_orders)
                buy_vol += bvol
                sell_vol += svol
                mm_orders = self.market_making_orders(Product.SQUID_INK, od, fair_value, next_bid, next_ask, position, buy_vol, sell_vol)
...
#########################################################
# Aggregated Trader Class (runs both strategies)
#########################################################

class Trader:
    def __init__(self, params: Dict = None):
        self.resin_trader = TraderResin(params.get(Product.RAINFOREST_RESIN, None) if params else None)
        self.squid_trader = Trader_SQUID_INK(params.get(Product.SQUID_INK, None) if params else None)
    ...
    def run(self, state: TradingState):
        ...
        total_conversions = 0
        if Product.RAINFOREST_RESIN in state.order_depths:
            resin_result, resin_conversions, resin_trader_data = self.resin_trader.run(state, traderObject[Product.RAINFOREST_RESIN])
            traderObject[Product.RAINFOREST_RESIN] = resin_trader_data
            result.update(resin_result)
            total_conversions += resin_conversions
        if Product.SQUID_INK in state.order_depths:
            squid_result, squid_conversions, squid_trader_data = self.squid_trader.run(state, traderObject[Product.SQUID_INK])
            traderObject[Product.SQUID_INK] = squid_trader_data
            result.update(squid_result)
            total_conversions += squid_conversions
        traderData = jsonpickle.encode(traderObject)
